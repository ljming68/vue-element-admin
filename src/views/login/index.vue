...
export default {
  ...
  data() {
    ...
    const validateMobile = (rule,value,callback) =>{
      validMobile(value) ? callback() : callback(new Error('手机号格式不正确')) // 三段式
    }
    ...
    return {
      ...
      // 表单验证
      // required 必填
      // trigger 触发方式 blur / change  不聚焦/ 改变
      // 自定义函数
      loginRules: {
  ...
  methods: {
    ...
    handleLogin() {
      ...
      // this.$store.dispatch('user/login','666')
      // ref 可以获取一个元素的dom对象
      //  ref 作用到组件上的时候，可以获取到这个组件的实例 this
      this.$refs.loginForm.validate( async isOk => {
        console.log(isOk)
        if(isOk){
         try {
           this.loading = true
            // 通过校验 this['user/login'] 拿到方法
            await this['user/login'](this.loginForm)
            // 登录成功之后
            // async 标记的函数实际上是一个promise
